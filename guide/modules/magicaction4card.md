...
[Related concepts](../concept.md#2-Card/Node, Parent/Child Card, Ancestor Card, Descendant Card) of parent and child nodes.
...
4. Synchronize multi-window profile: OhMyMN does not synchronize the profile immediately in multi-window cases. You can force synchronization with this option.
...
Although OhMyMN's selection is not the same as MN's selection, you can't use the delete or cut function. However, there is always a way. You can add a special tag to the selected card and then filter it by the tag to quickly reselect it. Finally, you can delete this tag.

Similarly, OhMyMN cannot be filtered by color. You can add a special tag after filtering with MN, then filter by OhMyMN and process it. Finally, just remove this tag.

### Merge Multiple Card

MN's merge feature is lacking. It cannot merge titles and it merges titles into cards as comments. This feature solves three problems:
...
::: tip Update
v4.1.0 Improved: Extracted images are not automatically OCR merged.
:::
...
- Swap Title and Excerpt: In the case where both the title and the excerpt are present, the two are swapped. The case where only one is present is the same as `Switch to Non-Existent`.
